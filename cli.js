...
function init() {
	...
	const to_path = './ioncss.config.js';
	if (fs.existsSync(to_path)) {
		...
		fs.copyFile(from_path, to_path, function (err) {
			if (err) {
				...
				console.log('Файл конфигурации ioncss.config.js успешно создан');
			}
		});
	}
}
...
async function generate() {
	...
	if (fs.existsSync('./ioncss.config.js')) {
		const user_config = (await import(pathToFileURL('./ioncss.config.js').href)).default;
